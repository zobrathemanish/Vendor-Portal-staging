...
single_product_bp = Blueprint("single_product", __name__)

@single_product_bp.route('/single-product', methods=['GET'])
@login_required
...
@single_product_bp.route('/single-product', methods=['POST'])
@login_required
def submit_single_product():
    ...
    if action == "generate":
        if not price_rows:
            ...
            return redirect(url_for('single_product.single_product_page'))

        output_dir = os.path.join(current_app.config['UPLOAD_FOLDER'], "single_product_batches")
        ...
        return redirect(url_for('single_product.single_product_page', generated = 1))
...
@single_product_bp.route('/download-single-products-excel')
